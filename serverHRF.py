...
def _stream_samples(sdr, client_socket, buffer_size, duration_seconds, start_time):
    ...
    end_time = float('inf') if duration_seconds == 0 else start_time + duration_seconds
    initialization_timeout = 10
    initialization_start = time.time()
    samples_sent = 0

    logging.info(f"Starting sample streaming {'continuously' if duration_seconds == 0 else f'for {duration_seconds} seconds'}")
...
def main(args):
    ...
    def initialize_hackrf():
        ...
        for attempt in range(max_retries):
            try:
                ...
                logging.error(f"Failed to initialize HackRF device (Attempt {attempt + 1}/{max_retries}): {e}")
    ...
    try:
        while True:
            sdr = initialize_hackrf()
            if not sdr:
                logging.error("Could not initialize HackRF. Retrying in 10 seconds...")
                time.sleep(10)
                continue
